    ...
    <div class="app-container">
        ...
        <div class="box-contnet-wrap">
            <el-row :gutter="10" class="mb8">
                <el-col :span="1.5">
                    <el-button type="danger" size="mini" :disabled="multiple" v-hasPermi="['report:callquality:export']" @click="handleExport">导出
                    </el-button>
                </el-col>
                <el-col :span="1.5">
                    <el-button type="success" size="mini" v-hasPermi="['report:callquality:exportAll']" @click="handleExportAll">全选导出
                    </el-button>
                </el-col>
            ...
            <el-table v-loading="loading" max-height="550" :data="caseList" ref="multiTable" :row-key="getRowKeys" @selection-change="handleSelectionChange">
                ...
                <el-table-column label="坐席是否有敏感词" width="180" prop="isSeatWord" >
                    <template slot-scope="scope" v-if="scope.row.isSeatWord != null">
                        <span>{{scope.row.isSeatWord=='1'?'是':scope.row.isSeatWord=='0'?'否':''}}</span>
                    </template>
                </el-table-column>
                <el-table-column label="坐席敏感词" width="200" prop="seatWord" />
                <el-table-column label="客户是否有敏感词" width="180" prop="isClientWord" >
                    <template slot-scope="scope" v-if="scope.row.isClientWord != null">
                        <span>{{scope.row.isClientWord=='1'?'是':scope.row.isClientWord=='0'?'否':''}}</span>
                    </template>
                </el-table-column>
        ...
        methods: {
            ...
            getList(type) {
                ...
                if(type == 1){
                    ...
                    operationApi.callQualitylist(this.searchParams).then((response) => {
                        this.clearSelection();
            ...
            handleChange2(value) {
                //console.log(value)
                if (value == null) {
                    this.queryParams.startQualityTime = "";
                    this.queryParams.endQualityTime = "";
                } else {
                    this.queryParams.startQualityTime = value[0];
                    this.queryParams.endQualityTime = value[1];
                }
            },
            recordingPlay(val) {
