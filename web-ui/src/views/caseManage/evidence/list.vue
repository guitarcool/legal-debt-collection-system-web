        ...
        <search-bar v-show="showSearch" :model="queryParams" :has-filter="hasFilter" :flag="true" @submit="getList(1)"
            ...
            <template #default>
                ...
                <el-form-item label="订单号：" prop="id">
                    <el-input clearable v-model="queryParams.caseId" placeholder="请输入订单号" size="small" style="width: 200px"
        ...
        <div class="box-contnet-wrap">
            ...
            <el-table v-loading="loading" :data="caseList" ref="multiTable" :row-key="getRowKeys" @selection-change="handleSelectionChange">
                ...
                <el-table-column label="分案名" prop="divisionName" :show-overflow-tooltip="true" fixed="left"
                    width="100" />
                <el-table-column label="创建时间" prop="createTime" width="120" :render-header="renderTime">
...
export default {
  name: "list",
  components: { SearchBar, evidenceImport, exportDialog },
  data() {
    return {
      // 遮罩层
      loading: false,
  ...
  methods: {
    ...
    handleUpdate(item) {
      this.$router.push({ name: "infoList", query: { pid: item.id } });
    },
    handleChange(value) {
      //console.log(value)
      if (value == null) {
