      ...
      <div class="add-template">
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12" v-if="type == 2 || type == 3" style="margin-bottom: 10px">
            ...
            <el-tree style="overflow-y: auto;height: 300px;" :data="templateData" class="border-style" node-key="id" :expand-on-click-node="false"
        ...
        <ul class="drag-box" ref="parentNode">
          ...
          <!-- <li v-else
                    ...
                    >
                        <p>{{item.label}}</p>
                        <p v-if="item.fileSort != null">{{item.fileSort}}</p>
                        <div>
                            <i class="el-icon-delete" @click="deleteData(index)"></i>
                        </div>
                    </li> -->
          <li v-else v-for="(item,index) in list" :key="index" class="item"
            ...
            <el-form class="model_form" ref="isSignform" :model="isSignform">
              ...
              <div style="width:40%">
                <el-form-item label="" prop="placeContent" v-if="item.isSign!=0&&item.signSetUp ==1">
                  <el-input v-model="item.placeContent" placeholder="请输入关键字，用逗号分隔多个关键字。"></el-input>
                </el-form-item>
                <el-form-item label="" prop="pageRange" v-if="item.isSign!=0&&item.pagePlace">
                  <el-input v-model="item.pageRange" placeholder="请输入页码或用逗号分隔的页面范围。"></el-input>
                </el-form-item>
              </div>
            </el-form>
          </li>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      submit() {
        ...
        this.$confirm(`此操作连接数据库，是否继续?`, "提示", {
          ...
          this.list.forEach((item, i) => {
            ...
            if (!item.pageRange) {
              formData.append('placeContent', item.placeContent ? item.placeContent : '')
            }
      ...
      //     type: "warning",
      //   }).then(() => {
      //     console.log('1213423434')
      //     let formData = new FormData();
      //     formData.append("id", that.id);
      //     if (that.list.length > 0 && that.list[0].file != null) {
      //       formData.append("file", that.list[0].file);
      //       //电子签章内容
      //       formData.append(`isSign`, item.isSign ? item.isSign : '')
      //       formData.append(`pagePlace`, item.pagePlace ? item.pagePlace : '')
      //       formData.append(`signName`, item.signName ? item.signName : '')
      ...
      //       }
      //     }
      //     formData.append("type", that.original.type);
      //     formData.append("pdfName", that.list.length > 0 ? that.list[0].pdfName : that.original.pdfName);
      //     console.log(formData);
      //     //return
      //     const instance = axios.create({
      //       withCredentials: true
      //     });
      //     instance.defaults.headers.common['Authorization'] = 'Bearer ' + getToken()
      //     var url = '/template/info/templateFile/update';
      //     var these = that
      //     instance({
      //       method: 'post',
      //       url: process.env.VUE_APP_BASE_API + url,
      ...
      //       console.log(response)
      //       if (response.data.code == 200) {
      //         these.msgSuccess(response.data.message);
      //         these.dialogVisible = false;
      //         these.$emit('refresh');
      //       } else {
      //         these.msgError(response.data.message);
      //       }
      //     }).catch(error => {
      //       these.msgError(error);
      //     })
