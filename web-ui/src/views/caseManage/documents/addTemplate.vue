<template>
    <div>
            ...
            <template v-slot:default>
                    ...
                    >
                        <el-form-item prop="content" class="move-left">
                            <el-input
                                ...
                                :rows="8"
                                type="textarea"
                                placeholder="请在框内编辑模版，用“${字段英文名}”表示需要映射的字段，可点击“查看字段表”查看字段"
                            ></el-input>
                            <el-input
                ...
                <div class="add-template">
                    ...
                    >
                        ...
                        <el-form-item
                            ...
                            v-if="
                                form.formatType != '' &&
                                    Number(form.formatType) === 4
                            "
                        >
                            <el-switch
                                v-model="form.isSubLoop"
                                active-color="#13ce66"
                                inactive-color="#ff4949"
                                :active-value="1"
                                :inactive-value="0"
                ...
                >
                        ...
                        <el-col :span="12">
                            ...
                            >
                                <el-form-item class="move-left">
                                    <el-upload
                                        class="upload-demo covered-with"
                                        action="string"
                    ...
                    <div>
                        <div
                            class="add-template"
                            v-if="['0', '4'].includes(form.formatType)"
                        >
                            <p
                                v-if="
                                    (title == '新增模版' &&
                        ...
                        <ul class="drag-box" ref="parentNode">
                            ...
                            <draggable v-model="draggableList" v-else>
                                <transition-group>
                                    ...
                                    >
                                        <div
                                            style="width:100%;display:flex;align-items: center;margin: 10px 0;"
                                        >
                                            <p
                                                style="width:50%;font-size:16px;font-weight:700;color:#000;"
                                            >
                                                {{ item.label }}
                                            </p>
                                            <div
                                        ...
                                        >
                                            ...
                                            >
                                                <el-select
                                                    clearable
                                                    v-model="item.signSetUp"
                                                    filterable
                                                    placeholder="请选择"
                                                >
                                                    <el-option
                                                        v-for="item in signSetUpOptions"
                                                        :key="item.dictValue"
                                                        :label="item.dictLabel"
                                                        :value="item.dictValue"
                                                    >
                                        ...
                                        >
                                            ...
                                            >
                                                <el-select
                                                    clearable
                                                    filterable
                                                    v-model="item.pagePlace"
                                                    placeholder="请选择"
                                                >
                                                    <el-option
                                                        v-for="item in pagePlaceOptions"
                                                        :key="item.dictValue"
                                                        :label="item.dictLabel"
                                                        :value="item.dictValue"
                                                    >
                                            ...
                                            >
                                                <el-select
                                                    clearable
                                                    filterable
                                                    v-model="item.pageIndex"
                                                    placeholder="请选择"
                                                >
                                                    <el-option
                                                        v-for="item in pageIndexOptions"
                                                        :key="item.value"
                                                        :label="item.label"
                                                        :value="item.value"
                                                    >
                                            ...
                                            >
                                                <el-input
                                                    clearable
                                                    v-model="item.placeContent"
                                                    placeholder="请输入关键字，用逗号分隔多个关键字。"
                                                ></el-input>
                                            </el-form-item>
                                            <el-form-item
                                                style="width:26%"
                                            ...
                                            >
                                                <el-input
                                                    clearable
                                                    v-model="item.pageRange"
                                                    placeholder="请输入页码或用逗号分隔的页面范围。"
                                                ></el-input>
                                            </el-form-item>
                                        </el-form>
                                    </li>
        ...
        <editTemplate
            ...
            :tid="editData.tid"
            @refresh="refresh"
            :singleData="editData.singleData"
        >
        </editTemplate>
    </div>
</template>
...
import { addGetUsers } from "@/api/system/role";
    ...
    methods: {
            ...
            if (this.id) {
                templateApi.info(this.id).then(res => {
                    ...
                    if (this.form.manager) {
                        if (
                            this.userList.some(
                                tag => tag.userId == this.form.manager
                            )
                        ) {
                    ...
                    //文件信息
                    if (
                        this.form.templateFileList &&
                        this.form.templateFileList.length
                    ) {
                        for (
                            var i = 0;
                            i < this.form.templateFileList.length;
                            i++
                        ) {
                            this.form.templateFileList[
                                i
                            ].signId = this.form.templateFileList[
                                i
                            ].signId?.toString();
                            this.form.templateFileList[i].label =
                                this.form.templateFileList[i].fileName != null
                                    ? this.form.templateFileList[i].fileName
                                    : this.form.templateFileList[i].pdfName;
                        }
                    }
        ...
        submit() {
            ...
            if (!this.form.templateType) {
                this.msgError("请选择模版类别");
                return;
            }
            if (!this.form.formatType) {
                this.msgError("请选择格式类型");
                return;
            }
            ...
            formData.append(
                "manager",
                this.managerId ? this.managerId : this.form.manager
            );
            ...
            this.draggableList.forEach((item, i) => {
                ...
                if (item.signSetUp == 2 && item.pageIndex == "All") {
                    ...
                    formData.append(
                        `list[${i}].pageRange`,
                        item.pageRange ? item.pageRange : ""
                    );
                } else if (item.signSetUp == 1) {
                    formData.append(
                        `list[${i}].placeContent`,
                        item.placeContent ? item.placeContent : ""
                    );
                }
            });
            //return
